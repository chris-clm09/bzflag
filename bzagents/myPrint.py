...
class PFPrinter(object):
    
    def __init__(self, fileName=FILENAME):
        self.outfile = open(fileName, 'w')
        print >>self.outfile, gnuplot_header(-WORLDSIZE / 2, WORLDSIZE / 2)
        
    def printObstacles(self, obstacles = OBSTACLES):
        print >>self.outfile, draw_obstacles(obstacles)

    def printPotentialFields(self, field_function):
        if field_function is None:
            field_function = generate_field_function(150)
        print >>self.outfile, plot_field(field_function)

    ########################################################################
    # Plot the potential fields to a file
if __name__ == '__main__':
    test = PFPrinter('junk.gpi')
    test.printObstacles()
    test.printPotentialFields(None)